		...
		return this.each(function() {
			...
			function buildResults(resultObjects, sFilterTxt) {
				...
				for (i = 0; i < resultObjects.length; i += 1) {
					...
					$(anchor).append('<span class="text">' + text + '</span>');

					if (typeof resultObjects[i].image === 'string') {
						$(anchor).prepend('<img src="' + resultObjects[i].image + '" />');	
					}
					
					if(resultObjects[i].extra){
						$(anchor).append('<br /><span class="extra">' + resultObjects[i].extra + '</span>');
					}
